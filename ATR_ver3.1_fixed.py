...
def get_ma60(ticker):
    """60일 이동 평균선 조회"""
    dk = pyupbit.get_ohlcv(ticker, interval="minute1", count=61)
    ma60 = dk['close'].rolling(60).mean().iloc[-1]
    return ma60

def get_ma120(ticker):
    """120일 이동 평균선 조회"""
    dj = pyupbit.get_ohlcv(ticker, interval="minute1", count=121)
    ma120 = dj['close'].rolling(120).mean().iloc[-1]
    return ma120
        ...
        if buy_flag == False and sell_flag == False and hold_flag == False:
            ...
            while True:
                if buy_flag == True and sell_flag == False and hold_flag == False:
                    ...
                    ma120 = get_ma120(ticker)
                    curr_price = get_current_price(ticker)
                    up_price = target_price + ma20
                    # cut_price = ma20 + (ma20 * 0.003)
                    # or curr_price <= cut_price
                    print("[현재가 : ", curr_price, "]", "[목표가 : ", round(up_price, 2), "]", "[20일선 : ", round(ma20, 1), "]",
                          "[60일선 : ", round(ma60, 1), "]", "[120일선 : ", round(ma120, 1), "]")
                    if up_price > curr_price or ma20 < ma60 or ma60 < ma120 or ma20 < ma120:
                        time.sleep(0.2)
                        continue
                    else:
