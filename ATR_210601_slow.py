...
while True:
    try:
        if buy_flag == False and sell_flag == False and hold_flag == False:
            curr_price = get_current_price("KRW-ETH")
            close_price = get_close("KRW-ETH")
            if buy_flag == False:
                print(curr_price)
                buy_flag = True
            time.sleep(0.3)
            ...
            while True:
                if buy_flag == True and sell_flag == False and hold_flag == False:
                    ...
                    curr_price = get_current_price("KRW-ETH")
                    up_price = target_price + close_price
                    print("[현재가 : ", curr_price, "]", "[목표가 : ", round(up_price, 0), "]", "[20일선 : ", ma20, "]")
                    if up_price > curr_price:
                        time.sleep(0.3)
                        continue
                    else:
                        cash = upbit.get_balance("KRW")
                        order = upbit.buy_market_order("KRW-ETH", cash * 0.9995)
                        sell_flag = True
                        break
            ...
            while True:
                if buy_flag == True and sell_flag == True and hold_flag == False:
                    ...
                    if ma20 < curr_price:
                        ...
                        volume = upbit.get_balance("ETH")
                        ret = upbit.sell_market_order("KRW-ETH", volume)
                        hold_flag = True
                        time.sleep(0.3)
                        break
        ...
        if hold_flag == True:
            uncomp = upbit.get_order("KRW-ETH")
