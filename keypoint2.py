...
class Consumer(threading.Thread):
    def __init__(self, q):
        ...
        self.ma30.extend(df['close'])
        self.ma60.extend(df['close'])
        self.high1.extend(df['high'])
        self.low1.extend(df['low'])
        self.close1.extend(df['close'])

        print(len(self.ma10), len(self.ma30), len(self.ma60), len(self.high1), len(self.low1), len(self.close1))
        ...
        while True:
            try:
                if not self.q.empty():
                    ...
                    curr_ma30 = sum(self.ma30) / len(self.ma30)
                ...
                if buy_flag == False and sell_flag == False and wait_flag == False and \
                    price_curr >= price_buy and curr_ma10 >= curr_ma30 and \
                    curr_ma60 <= curr_ma30 and price_curr >= target_price :
                ...
                # 3 minutes
                if i == (5 * 60 * 3):
                    print(f"[{datetime.datetime.now()}] 현재가 {price_curr}, 목표가 {price_buy}, ma {curr_ma10:.2f}/{curr_ma30:.2f}/{curr_ma60:.2f}, buy_flag {buy_flag}, sell_flag {sell_flag}, wait_flag {wait_flag}")
                    i = 0
                i += 1
            except:
                print("error")
